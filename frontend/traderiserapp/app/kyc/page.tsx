...
// Match TopNavbar's User interface
interface User {
  username: string;
  email: string;
  image?: string;
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (userSession) {
          const parsedUser: User = JSON.parse(userSession);
          setUser(parsedUser);
          setAccounts(parsedUser.accounts || []);

          const activeId = localStorage.getItem("active_account_id");
          const foundAccount = parsedUser.accounts?.find(
            (acc: Account) => String(acc.id) === activeId
                    ...
                    <div className="border border-dashed border-white/30 rounded-2xl p-6 text-center hover:border-amber-500/50 transition-colors">
                      ...
                      <label htmlFor="id-upload" className="cursor-pointer block">
                        ...
                        <p className="text-white/80 text-sm">
                          {idFile ? idFile.name : "Upload Passport or National ID"}
                        </p>
                      </label>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="border border-dashed border-white/30 rounded-2xl p-6 text-center hover:border-amber-500/50 transition-colors">
                      ...
                      <label htmlFor="selfie-upload" className="cursor-pointer block">
                        <Upload className="w-8 h-8 mx-auto text-white/60 mb-2" />
                        <p className="text-white/80 text-sm">
                          {selfieFile ? selfieFile.name : "Clear selfie while holding your ID"}
                        </p>
                      </label>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="border border-dashed border-white/30 rounded-2xl p-6 text-center hover:border-amber-500/50 transition-colors">
                      ...
                      <label htmlFor="address-upload" className="cursor-pointer block">
                        <Upload className="w-8 h-8 mx-auto text-white/60 mb-2" />
                        <p className="text-white/80 text-sm">
                          {addressFile ? addressFile.name : "Utility bill or bank statement"}
                        </p>
                      </label>
                    </div>
                  </div>
