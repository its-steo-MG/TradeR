...
export default function TradingPage() {
  ...
  useEffect(() => {
    const fetchMarkets = async () => {
      ...
      if (!token) {
        ...
        try {
          ...
          if (Array.isArray(rawData)) {
            ...
            marketList = volatilityMarkets.length > 0 
              ? volatilityMarkets 
      ...
      if (savedMarket) {
        setSelectedMarketId(savedMarket.id);
  ...
  const handlePlaceTrade = useCallback(
    async (kind: DigitContractKind) => {
      ...
      try {
        ...
        if (responseData && typeof responseData === "object" && responseData !== null) {
          ...
          if (Array.isArray(rd.trades) && rd.trades.length > 0) {
            trade = rd.trades[0] as DigitTradeData;
          } else if (rd.trade && typeof rd.trade === "object") {
            trade = rd.trade as DigitTradeData;
          } else {
            trade = rd as DigitTradeData;
          }
        }
    ...
    [marketsLoaded, selectedMarketId, stake, isTrading, barrier, current, selectedMarket, balance]
  );
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col">
      ...
      <div className="flex-1 flex justify-center overflow-y-auto pb-20 px-2">
        <div className="w-full max-w-md md:max-w-lg lg:max-w-3xl xl:max-w-4xl px-3 py-4">
          {tab === "trade" ? (
            <div className="space-y-5">
              <div className="relative h-52 md:h-64 lg:h-80 rounded-3xl bg-slate-900 border border-slate-700 overflow-hidden">
                ...
                <PriceChart ticks={ticks} current={current} />
              </div>
              ...
              <div className="space-y-5 px-1">
                ...
                {isTrading && <div className="text-center py-4 text-blue-400 font-medium">Placing trade...</div>}
                {!marketsLoaded && <div className="text-center py-4 text-blue-400 text-sm">Loading markets...</div>}
              </div>
            </div>
          ) : (
            <PositionsPanel
      ...
      {showRobotPanel && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div
            className="w-full max-w-lg md:max-w-xl lg:max-w-2xl bg-slate-900 border border-slate-700 rounded-3xl overflow-hidden max-h-[95vh] flex flex-col shadow-2xl"
