...
def sync_wallet_to_mpesa(sender, instance, **kwargs):
    """Sync completed wallet transactions to M-Pesa Simulator with consistent mpesa_id"""
    ...
    try:
        ...
        if instance.transaction_type == 'withdrawal':
            ...
            txn = MpesaTransaction(
                ...
                reference=instance.reference_id,          # Use real reference
                recipient_name='SASHITRENDY TECHNOLOGIES',
                recipient_phone='5515738',
                category='business',
            )
            txn._wallet_reference_id = instance.reference_id   # Pass for ID generation
            txn.save()

            logger.info(f"Synced withdrawal {instance.id} → M-Pesa deposit | ID: {txn.mpesa_id}")
            ...
            logger.info(f"Synced deposit {instance.id} → M-Pesa withdrawal | ID: {txn.mpesa_id}")
