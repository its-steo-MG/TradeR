...
def get_user_access_token(request):
    """Helper to get valid access token or return error message"""
    try:
...
class DerivOAuthLoginView(APIView):
    """Step 1: Generate Deriv OAuth URL with PKCE"""
    ...
    def get(self, request):
        ...
        auth_url = (
            ...
            f"&client_id={settings.DERIV_APP_ID}"          # Make sure this is in settings.py
...
class DerivOAuthCallbackView(APIView):
    """Step 2: Handle redirect from Deriv after user authorizes"""
    permission_classes = [IsAuthenticated]
    ...
    async def get(self, request):
        ...
        # Save or update user's Deriv account
...
class DerivSellView(APIView):
    """New: Sell / Close an open contract"""
    permission_classes = [IsAuthenticated]
...
class DerivOpenContractView(APIView):
    ...
    async def post(self, request):
        ...
        subscribe = request.data.get("subscribe", True)   # default to subscribe
